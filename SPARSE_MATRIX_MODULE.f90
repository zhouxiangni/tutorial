...
PUBLIC::SPARSITY_PATTERN_INITIALIZE, &
    ...
    TYPE, PUBLIC::SPARSE_MATRIX
        ...
        REAL(dp), DIMENSION(:), POINTER::MATRIX_ENTRY=>NULL()
    ...
    SUBROUTINE SPARSITY_PATTERN_INITIALIZE(PATTERN, ROW, COL, MAXROWLEN0)
        TYPE(SPARSITY_PATTERN), INTENT(INOUT)::PATTERN
        INTEGER, INTENT(IN)::ROW, COL
        INTEGER, INTENT(IN)::MAXROWLEN0

        INTEGER::I, J, MAXROWLEN
        
        CALL SPARSITY_PATTERN_DESTROY(PATTERN)

        IF (ROW == COL) THEN
            MAXROWLEN = MAX(1, MAXROWLEN0)
        END IF
    ...
    SUBROUTINE SPARSE_MATRIX_FULL(M1, M2)
        TYPE(SPARSE_MATRIX), INTENT(IN)::M1
        REAL(dp), DIMENSION(:,:), INTENT(INOUT)::M2
    ...
    ! Gauss-Seidel iteration for certain times. The matrix is assumed to be square.
    ...
    SUBROUTINE SPARSE_MATRIX_MULTIPLE(M1, M2, M3)
        ...
        INTEGER::I, J, K, L, M
        REAL(dp), DIMENSION(1:M2%PATTERN%N_COLUMN)::A
