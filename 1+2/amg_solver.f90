!	algebric multigrid solve module
!	the module is used to solve a positive definite linear system with algebric
!	multigrid method. the solver can be refined by appointing smoothing steps
!	and smoother. now the default smoother is Gauss-Sidel iteration.
...
MODULE AMG_SOLVER_MODULE
  ...
  TYPE, PUBLIC::AMG_SOLVER
     ...
     REAL, DIMENSION(:,:), POINTER::MAX_PROJECTED_MATRIX=>NULL()
  END TYPE AMG_SOLVER
...
CONTAINS
  ...
  SUBROUTINE AMG_SOLVER_SOLVE(SOLVER, SOLUTION, RHS)
    TYPE(AMG_SOLVER), INTENT(INOUT)::SOLVER
    REAL, DIMENSION(:), TARGET, INTENT(INOUT)::SOLUTION
    REAL, DIMENSION(:), TARGET, INTENT(INOUT)::RHS
    ...
    DO
       DO I=1,SOLVER%N_PROJECT_COUNT-1
          ...
          CALL SPARSE_MATRIX_VECTOR_MULT_PS(SOLVER%PROJECTED_MATRIX(I)%PTR, SOLVER%PROJECTED_SOLUTION(I)%PTR, -1.0E0, TEMP(I)%PTR)
          CALL SPARSE_MATRIX_VECTOR_MULTIPLE(SOLVER%LEFT_PROJECT_MATRIX(I)%PTR, TEMP(I)%PTR, SOLVER%PROJECTED_RESIDUAL(I+1)%PTR)
  ...
  SUBROUTINE AMG_SOLVER_GAUSS_ELIMINATION(MATRIX, X, R, N)
    ...
    DO I=1,N-1
       ...
       DO J=I+1,N
          A = M(J,I)/M(I,I)
          M(J,I+1:N+1) = M(J,I+1:N+1) - A*M(I,I+1:N)
       END DO
    END DO
    DO I=N,1,-1
       DO J=I+1,N
